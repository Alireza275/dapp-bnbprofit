...
const CONTRACT_ADDRESS = '0x729a29c9ce72efb4f5a26764df476428b7ec8019'
const DEFAULT_REFERRAL = '0x8eD9B79ae34642ba1a723E83670Cc12dD5D4A470'

const DEPOSIT_PERIOD_MIN = 6
const DEPOSIT_TOTAL_PROFIT_MIN = 120
...
function checkUserWallet(){
    ...
    if(wallet === "ethereum" && !isMetaMaskInstalled()){
        ...
        console.log(wallet, isMetaMaskInstalled())
        return;
    }
}
...
async function checkUserChainId() {
    ...
    if(wallet === "ethereum" && window.ethereum.chainId && (Number.parseInt(window.ethereum.chainId) !== 56)){
        ...
        {
            console.log(wallet+"   "+window.BinanceChain);
        }
}
...
function invest (contract, accountBalance, dashboardObserver) {
    ...
    console.log(tariffId, getReferralFromStoreOrLink(), {value},INVEST_BUTTON_CONTENT_ON_TRANSACTION_RUNNING)
...
function withdraw (contract, dashboardObserver) {
    ...
    console.error(dashboardObserver);
